...
test('avaliações públicas isoladas, validação, cookie e atualização sem duplicação',async()=>{
 ...
 const votes={createIndex:async(keys,options)=>assert.equal(options.unique,true),findOne:async f=>rows.find(r=>r.projetoId===f.projetoId&&r.visitante===f.visitante),updateOne:async(f,u)=>{const r=rows.find(r=>r.projetoId===f.projetoId&&r.visitante===f.visitante);if(r)Object.assign(r,u.$set);else rows.push({...f,...u.$set});},aggregate:p=>({toArray:async()=>{const selected=rows.filter(r=>r.projetoId===p[0].$match.projetoId);return selected.length?[{total:selected.length,media:selected.reduce((s,r)=>s+r.nota,0)/selected.length}]:[];}})};
 const database={getCollection:async name=>{collections.push(name);if(name==='avaliacoesVisitantes')return votes;if(name==='projetos')return {findOne:async f=>f._id.toString()===id?{_id:id}:null};throw Error('Coleção da banca não pode ser acessada');}};
 const app=express();app.use(express.json());app.use('/votes',await createRouter(database));
 ...
 try{
  let response=await fetch(base+id);const cookie=response.headers.get('set-cookie').split(';')[0];assert.match(response.headers.get('set-cookie'),/HttpOnly/);assert.equal((await response.json()).data.total,0);
